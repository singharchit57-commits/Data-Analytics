...
CREATE TABLE Student_Performance (student_id INT PRIMARY KEY, name VARCHAR(50), course VARCHAR(30),
score INT, attendance INT, mentor VARCHAR(50), join_date DATE, city VARCHAR(50));
...
SELECT student_id, name, score, LAG(score, 1) OVER (ORDER BY score DESC) AS previous_score
FROM Student_Performance
ORDER BY score DESC;

-- 3. Convert all student names to uppercase and extract the month name from join_date.
SELECT  UPPER(name) AS name_uppercase, MONTHNAME(join_date) AS join_month
FROM Student_Performance;

-- 4.Show each student's name and the next student’s attendance (ordered by attendance).
SELECT  name, attendance, LEAD(attendance, 1) OVER (ORDER BY attendance) AS next_attendance
FROM Student_Performance
ORDER BY attendance;
